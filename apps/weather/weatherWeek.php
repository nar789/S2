<?php
  ...
  $generatedDate=array();
  for($i=0;$i<7;$i++){
    array_push($generatedDate,mktime(0,0,0,date("m"),date("d")+$date+$i,date("y")));
  }
...
<html lang="en">
  ...
  <body>
    ...
    <center>
      <?php
        for($i=0;$i<7;$i++){
          echo '<div id=box><div id=date>'.date("M d D",$generatedDate[$i]).'</div>
          <div id=weather_icon>';

          if(date("a")=="am"){
            echo $cssIcon[$forecast_comment[$i][0]];
          }else{
            echo $cssIcon[$forecast_comment[$i][1]];
          }
          echo '</div>
          ...
          if(date("a")=="am"){
            echo $forecast_temp[$i][0];
          }else{
            echo $forecast_temp[$i][1];
          }
